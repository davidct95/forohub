...
import com.alura.forohub.domain.respuesta.validations.create.ValidarRespuestaCreada;
import com.alura.forohub.domain.respuesta.validations.update.ValidarRespuestaActualizada;
...
public class RespuestaController {
    ...
    public ResponseEntity<DetalleRespuestDto> crearRespuesta(@RequestBody @Valid CrearRespuestaDto crearRespuestaDTO,
        ...
        return ResponseEntity.created(uri).body(new DetalleRespuestDto(respuesta));

    }

    @GetMapping("/topico/{topicoId}")
    @Operation(summary = "Lee todas las respuestas del tema dado")
    public ResponseEntity<Page<DetalleRespuestDto>>
    leerRespuestaDeTopico(@PageableDefault(size = 5, sort = {"ultimaActualizacion"},
        ...
        var pagina = respuestaRepository.findAllByTopicoId(topicoId, pageable).map(DetalleRespuestDto::new);
    ...
    leerRespuestasDeUsuarios(@PageableDefault(size = 5, sort = {"ultimaActualizacion"},
        ...
        var pagina = respuestaRepository.findAllByUsuarioId(usuarioId, pageable).map(DetalleRespuestDto::new);
        return ResponseEntity.ok(pagina);
    }

    @GetMapping("/{id}")
    @Operation(summary = "Lee una única respuesta por su ID")
    public ResponseEntity<DetalleRespuestDto> leerUnaRespuesta(@PathVariable Long id){
    ...
    public ResponseEntity<DetalleRespuestDto> actualizarRespuesta(@RequestBody @Valid ActualizarRespuestaDto actualizarRespuestaDTO, @PathVariable Long id){
